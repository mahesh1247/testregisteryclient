...
public class TestregisteryclientApplication {
	@Bean
	CommandLineRunner runner (ReservationRepository reservationRepository){
		return string ->{
			Stream.of("Mahesh","Test","Abs","Sureh"
					,"Nani","Technical","class","Object","Variable","KeyWords","statements",
					"Hypermedia","REST","Microservice","SOA","Patterns","Functional",
					"LISP","Assemblers","ByteCode","Class Level Variables","Local Variables").forEach(n -> reservationRepository.save(new ReservationDetials(n,String.valueOf(Math.random()))));
		};
	
	}
	public static void main(String[] args) {
		SpringApplication.run(TestregisteryclientApplication.class, args);
	}
}
